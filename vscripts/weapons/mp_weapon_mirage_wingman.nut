global function OnWeaponPrimaryAttack_weapon_mirage_wingman
global function OnProjectileCollision_weapon_mirage_wingman

#if SERVER
global function OnWeaponNpcPrimaryAttack_weapon_mirage_wingman
#endif // #if SERVER
...
var function OnWeaponPrimaryAttack_weapon_mirage_wingman( entity weapon, WeaponPrimaryAttackParams attackParams )
{
    _weapon = weapon
...
#if SERVER
var function OnWeaponNpcPrimaryAttack_weapon_mirage_wingman( entity weapon, WeaponPrimaryAttackParams attackParams )
{
    _weapon = weapon
    //EmitSoundOnEntity( weapon, "Mirage_PsycheOut_Activate_3P" )
...
void function OnProjectileCollision_weapon_mirage_wingman( entity projectile, vector pos, vector normal, entity hitEnt, int hitbox, bool isCritical )
{
    printt("Projectile hit -------------------------------" + hitEnt)
    #if SERVER
	if (hitEnt.IsWorld())
	    CreateHoloPilotDecoys_mirage_wingman( _weapon.GetWeaponOwner(), pos)
    #endif
}

#if SERVER
void function CreateHoloPilotDecoys_mirage_wingman( entity player,  vector offsetOrigin)
{
	Assert( player )
