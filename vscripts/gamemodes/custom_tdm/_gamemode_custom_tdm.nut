...
// purpose: clientcommand callback for "next_round"
...
// purpose: clientcommand callback for "tgive"
...
// purpose: main TDM function for handling game phases
...
// purpose: destroy all player-spawned props
...
// purpose: run the voting phase for all players
...
// purpose: internal function to register a new TDM location
...
// purpose: fade the player's screen to black and then return it to normal after a specified amount of time
...
// purpose: internal function for handling player connection
        ...
        void functionref() attackerHandleFunc = void function() : (victim, attacker, damageInfo)  {
            if( IsValid(attacker) && attacker.IsPlayer() && IsAlive( attacker ) && attacker != victim )
            {
                ...
                if ( IsGunGameMode() )
                {
                    ...
                    // take old weapons from the attacker so we can give the next tier of weapons
                    {
                        if( IsValid( primary ) )
                            attacker.TakeWeaponByEntNow( primary )
